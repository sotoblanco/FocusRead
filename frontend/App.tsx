...
const App: React.FC = () => {
  ...
  // Sync settings
  useEffect(() => {
    // Debounce or just fire and forget for now (could be optimized)
  ...
  useEffect(() => {
    if (view === 'reading' && activeSessionId) {
      const currentItem = library.find(i => i.id === activeSessionId);
      if (currentItem) {
        const updatedItem = {
          ...currentItem,
          currentIndex,
          ...
          elapsedTime,
          lastRead: Date.now(),
          chunks: chunks // Ensure chunks are up to date if they changed (e.g. formatting)
        };

        // Optimistic update
        setLibrary(prev => prev.map(item => item.id === activeSessionId ? updatedItem : item));

        // API Update
        api.updateStory(updatedItem).catch(console.error);
      }
    }
    ...
    if (correct) {
      if (currentIndex < chunks.length - 1) {
        ...
        setLibrary(prev => {
          ...
          const completedItem = updated.find(i => i.id === activeSessionId);
          if (completedItem) api.updateStory(completedItem);
          return updated;
        });
        setView('summary');
      }
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-500 ${getThemeClass()}`}>
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="flex space-x-2">
              <button
                ...
                className="p-2 rounded-full hover:bg-black/5 transition-colors text-indigo-500"
                title="Leaderboard"
              >
      ...
      {/* Logout Button (Fixed Top Left or in Settings? Let's put it in Settings for now or top left) */}
      {!showAuthModal && user && view === 'upload' && (
        <div className="fixed top-6 right-6 z-50 flex space-x-4">
          <button
            onClick={() => setShowLeaderboard(true)}
            className="px-4 py-2 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full text-xs font-bold uppercase tracking-widest text-indigo-600 transition-colors"
          >
            Leaderboard
          </button>
          <button
            onClick={handleLogout}
            className="px-4 py-2 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full text-xs font-bold uppercase tracking-widest text-gray-500 hover:text-red-500 transition-colors"
          >
